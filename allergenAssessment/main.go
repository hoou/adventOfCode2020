...
func matchAllergen(allergen *Allergen, meals []*Meal) *Ingredient {
	...
	for _, meal := range meals {
		...
		for ingredient := range meal.ingredients {
			...
			} else if ingredient.allergen == allergen {
				allergenAlreadyMatched = true
			}
		}
...
func filterMeals(meals []*Meal, allergenFilter *Allergen) []*Meal {
	var result []*Meal
	for _, meal := range meals {
		allergenFound := false
		for allergen := range meal.allergens {
...
func parseInput(input string) ([]*Meal, []*Allergen) {
	var meals []*Meal
	allergensMap := make(map[string]*Allergen)
	ingredientsMap := make(map[string]*Ingredient)
	...
	for _, mealRaw := range strings.Split(input, "\n") {
		...
		for _, allergen := range strings.Split(mealRawParts[3], ", ") {
			allergenPtr, allergenExists := allergensMap[allergen]
			if !allergenExists {
	...
	allergens := make([]*Allergen, len(allergensMap))
	i := 0
	for _, v := range allergensMap {
		allergens[i] = v
		i++
	}
	return meals, allergens
}
...
func (m *Meal) String() string {
	return "Meal{ingredients: " + fmt.Sprint(m.ingredients) + " allergens: " + fmt.Sprint(m.allergens) + "}"
}
