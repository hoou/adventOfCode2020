...
aaaabbb`
	...
	testRules, testWords := parseInput(testInput)
	testFirstSymbols := getFirstSymbols(testRules)
	testLanguage := buildLanguage(testFirstSymbols, testRules)
	sum := 0
	for _, word := range testWords {
		if _, ok := testLanguage[word]; ok {
			sum++
		}
	}
	...
	firstSymbols := getFirstSymbols(rules)
	language := buildLanguage(firstSymbols, rules)
	sum = 0
	for _, word := range words {
		if _, ok := language[word]; ok {
			sum++
		}
	}
	fmt.Println(sum)
...
func buildLanguage(firstSymbols []string, rules map[string][][]string) map[string]bool {
	result := make(map[string]bool)
	for _, firstSymbol := range firstSymbols {
		for _, rightPart := range rules[firstSymbol] {
			var rightPartTmpResult [][]string
			for _, symbol := range rightPart {
				if symbol[0] == '"' {
					result[symbol[1:len(symbol)-1]] = true
				} else {
					var words []string
					for word := range buildLanguage([]string{symbol}, rules) {
						words = append(words, word)
					}
					rightPartTmpResult = append(rightPartTmpResult, words)
				}
			}
			if rightPartTmpResult != nil {
				for _, word := range generateCombinations(rightPartTmpResult...) {
					result[word] = true
				}
			}
		}
...
func getFirstSymbols(rules map[string][][]string) []string {
	var result []string

	nonTerminalsOnRightSide := make(map[string]bool)

	for _, ruleRight := range rules {
		for _, rightPart := range ruleRight {
			for _, nonTerminal := range rightPart {
				if nonTerminal[0] != '"' {
					nonTerminalsOnRightSide[nonTerminal] = true
				}
			}
		}
	}

	for ruleLeft := range rules {
		if _, found := nonTerminalsOnRightSide[ruleLeft]; !found {
			result = append(result, ruleLeft)
		}
	}
...
func parseInput(input string) (map[string][][]string, []string) {
	...
	for _, ruleRaw := range strings.Split(inputParts[0], "\n") {
		...
		rules[ruleParts[0]] = rightParts
	}
	return rules, strings.Split(wordsRaw, "\n")
}

func generateCombinations(lists ...[]string) []string {
	if len(lists) == 0 {
		return []string{}
	} else if len(lists) == 1 {
		return lists[0]
		...
		if len(lists) == 2 {
			...
			return generateCombinations(args...)
		}
	}
}
