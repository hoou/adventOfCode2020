...
func Calculate(expression string) int {
	...
	stack.push(token)
	for {
		nextOperation = getNextOperation(stack, expression)
		if nextOperation == "$" {
			break
...
func getNextOperation(stack Stack, expression string) string {
	precedenceTable := map[string]map[string]string{
		"+": {"+": ">", "*": ">", "(": "<", ")": ">", "$": ">"},
		"*": {"+": "<", "*": ">", "(": "<", ")": ">", "$": ">"},
		"(": {"+": "<", "*": "<", "(": "<", ")": "<", "$": "-"},
		")": {"+": ">", "*": ">", "(": "-", ")": ">", "$": ">"},
	...
	stackToken, _ := stack.topNonOperand()
	inputToken, _ := getFirstToken(expression)
	if inputToken == nil {
		inputToken = &Token{"dollar", "$"}
	}
	if inputToken.name == "operand" {
		return "<"
	}
	return precedenceTable[stackToken.value.(string)][inputToken.value.(string)]
}
