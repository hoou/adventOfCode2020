...
func main() {
//	input2 := `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2017 ecl:blu byr:1942 hcl:#733820 eyr:2023 hgt:151cm pid:289923625`
	...
	for _, passport := range rawPassports {
		attrsCount := strings.Count(passport, ":")
		if attrsCount < 7 {
			continue
		}

		if attrsCount == 7 {
			if strings.Contains(passport, "cid:") {
				continue
			}
		}

		r := regexp.MustCompile("\\s")
		passport = r.ReplaceAllString(passport, ",")
		attrs := strings.Split(passport, ",")
		allAttrsValid := true
		for _, attr := range attrs {
			attrName := attr[0:3]
			attrValue := attr[4:]
			if !isAttrValid(attrName, attrValue) {
				allAttrsValid = false
				break
			}
		}
...
func isAttrValid(name string, value string) bool {
	switch name {
	case "byr":
		return isNumberRange(value, 1920, 2002)
	case "iyr":
		return isNumberRange(value, 2010, 2020)
	case "eyr":
		return isNumberRange(value, 2020, 2030)
	case "hgt":
...
func isValidHeight(value string) bool {
	...
	realValue := value[:len(value) - 2]
	switch unit {
	case "cm":
		return isNumberRange(realValue, 150, 193)
	case "in":
		return isNumberRange(realValue, 59, 76)
	}
	return false
}

func isNumberRange(value string, min int, max int) bool {
